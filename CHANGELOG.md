...
### Fixed
- **Multiple correct answers in context (Q#130 regression)**: Question "Which is even: 13, 24, 37, 48, 59?" had TWO even numbers (24 and 48), making it ambiguous. Added Rule 19 to detect when question's context allows multiple correct answers. Checks for: multiple even/odd numbers, multiple primes, multiple numbers divisible by X. If context has 2+ matches, rejects as ambiguous. Added 7 regression tests. Deleted Q130 from database
- **Nonsensical distractors for complex answers (Q#541 regression)**: 46+ questions (8.8% of database) had bad distractors ["0", "false", "False"] for Hebrew grammar, LaTeX, and inequality answers. Root cause: validation happened BEFORE distractor generation (validating placeholders only), and `_fallback_distractor()` returned generic hardcoded values. Implemented 3-layer defense: (1) New `_smart_fallback()` detects non-Latin scripts and returns None when impossible; (2) New Rule 18 `verify_distractor_quality()` catches type mismatches and multiple fallbacks; (3) Moved distractor generation into retry loop so failures trigger regeneration. Created `scripts/fix_bad_distractors.py` to clean database: fixed 35 questions with new logic, deleted 20 unfixable (Hebrew grammar, inequalities). Result: 565 MCQ → 523 after cleanup. All 522 tests pass
- **Answer in brackets bypass (Q#586 regression)**: Questions like "Which inequality does this number line represent? [x > -5]" had answer visible in brackets, bypassing validation. Root cause: Rule 5's `_answer_in_question_is_ok()` was too permissive, returning True for ALL "which" questions. Now: (1) rejects answers in brackets `[...]` immediately, (2) restricts "which" exemption to specific patterns (which is, which of), (3) enhances Rule 6 to catch `[x >`, `[x <` placeholder patterns. Added 7 regression tests covering comparative, math, and identification questions
